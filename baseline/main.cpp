...
int main(int argc, char** argv) {
  ...
  std::vector<int> catvector(allcategories,allcategories+batchesdispl[numproc]);
  ...
  if(myid==0){
    ...
    for(int k = 0; k < batchesdispl[numproc]; k++){
      int cat;
      fscanf(catf,"%d\n",&cat);
      //printf("cat %d %d %d\n",cat-1,allcategories[k],catvector[k]);
      if(cat-1!=catvector[k])
        pass = false;
    }
    fclose(catf);
    if(pass)
  ...
  if(myid==0){
    printf("BALANCE TIME: %e s\n",timebalance/numproc);
    printf(" KERNEL TIME: %e s\n",timekernel/numproc);
    printf("   COPY TIME: %e s\n",timecopy/numproc);
    printf("  OTHER TIME: %e s\n",timeinfer-(timebalance+timekernel+timecopy)/numproc);
